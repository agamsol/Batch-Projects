...
if not "!Webhook!"=="false" call "DiscordMsgPRO.exe" +silent --embed "Status of !HOST.count!" "!STARTUP.Message!" "!StartupColor!" "!EmbedImage!"
for %%a in (!HOSTS_LIST!) do (
    call :GET_HOST_STATUS "%%a"
    if !GetHost_STATUS! equ 1 (
...
for %%a in (!HOSTS_LIST!) do (
    if !Status[%%a]! equ 1 (
        ...
        if not "!Webhook!"=="false" call "DiscordMsgPRO.exe" +silent --embed !EmbedDetail! "!EmbedImage!"
        set Status[%%a]=0
    )
)
:: </Send Queued Messages>
