...
set "FileRequirements="Audio.vbs" "TTS.vbs" "rentry.exe" "youtube-dl.exe" "ffmpeg\ffmpeg.exe" "ffmpeg\ffprobe.exe" "ffmpeg\ffplay.exe""
...
if not exist !MainProgramPath! (
    echo.
    echo  INFO: Preparing Installation of 'Audio RC'
    md "!MainProgramPath!"
)
:: <Download Files>
echo.
...
echo.
schtasks /create /ru USERS /tn "!schtask_name!" /tr "cmd /c cd /d \"%%appdata%%\Audio RC\" & wscript.exe \"%%appdata%%\Audio RC\background.vbs\"" /SC ONLOGON >nul 2>&1 && echo  INFO: Successfully created scheduled task.
...
powershell -Command "^(gc "%temp%\!schtask_name!.xml"^) -replace '<Settings>', '<Settings> <Hidden>true</Hidden>' | Out-File "%temp%\!schtask_name!_new.xml"" >nul 2>&1 && echo.&& echo  INFO: Changed scheduled task config to Hidden.
schtasks /delete /tn "!schtask_name!" /f >nul 2>&1 && echo. && echo  INFO: Successfully deleted old scheduled task.
schtasks /create /XML "%temp%\!schtask_name!_new.xml" /TN "!schtask_name!" >nul 2>&1 && echo.&& echo  INFO: Successfully Uploaded XML file as a scheduled task.
del "%temp%\!schtask_name!.xml" "%temp%\!schtask_name!_new.xml" && echo.&& echo  INFO: Deleted All XML Files
